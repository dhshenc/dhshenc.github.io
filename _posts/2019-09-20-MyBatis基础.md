...
>       3. Mapper接口方法的输入参数类型和XxxMapper.xml中定义的每个sql 的parameterType的类型相同(注意：Mapper.xml中parameterType可不写，如果不写则由Mapper接口中的方法参数类型决定)  
>       4. Mapper接口方法的返回值类型和XxxMapper.xml中定义的每个sql的resultType（输出参数类型）的类型相同  
